...
tH.Units = "inches";
aspectRatio = 3/5;
tH.OuterPosition = [0 0 3.6, 3.6 * aspectRatio];
aH = nexttile;
...
% 
% if exportfig
exportgraphics(aH, 'proposalFigs/bubble_histogram.png', 'Resolution', 600);
% end
...
% % exportgraphics(fH, './proposalFigs/n_gt.png', 'Resolution', 600);
close all
% Get simulation result
f_sim = 100 : 100 : 9e3;
...
aH.YLim = [1.395, 1.56];
aH.FontName = 'Microsoft Yahei'; 
aH.YAxis(1).FontName = 'Microsoft Yahei';
xlabel('频率 (kHz)')
ylabel('声速 (km/s)')
...
exportgraphics(fH, './proposalFigs/simulation.png', 'Resolution', 600);
...
tH.OuterPosition = [0 0 16.7 16.7/8*2.5];
aH = nexttile;
...
xlim([0.3, 2.2])
legend(['BSD真值, ', num2str(sum(n_gt)), ' 个气泡'], ...
       ['全局优化方法, ', num2str(round(sum(n_ga_i))), ' 个气泡, MAE: ', num2str(mean(abs(n_gt(:) - n_ga_i(:))))], ...
       ['最小二乘方法, ', num2str(round(sum(n_i_lse))), ' 个气泡, MAE: ', num2str(mean(abs(n_gt(:) - n_i_lse(:))))], ...
       ['约束最小二乘方法, ', num2str(round(sum(n_i_clse))), ' 个气泡, MAE: ', num2str(mean(abs(n_gt(:) - n_i_clse(:))))])

if exportfig
exportgraphics(fH, './proposalFigs/fig_simulation-result.png', 'Resolution', 600);
end
